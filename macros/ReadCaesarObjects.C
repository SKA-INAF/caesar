...
int ReadCaesarObjects(std::string caesarFileName,std::string regionFileName="",std::string outputFileName="output.root",std::string regionOutFileName="ds9.reg",char delimiter='\t')
{
	//===================================
	//==       GET ARGS
	//===================================
	gCatalogFileName= caesarFileName;
...
int ReadObjects()
{
	...
	int status= AstroObjectParser::ParseCaesarData(gAstroObjects,gCatalogFileName,gDelimiter);
	if(status<0){
