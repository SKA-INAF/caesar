.. caesar documentation master file, created by
   sphinx-quickstart on Mon Dec  3 19:28:42 2018.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to caesar's documentation!
==================================

CAESAR is a C++ software tool for automated source finding in astronomical maps. It is distributed for research use only under the GNU General Public License v3.0. 


.. toctree::
   :caption: INSTALLATION
   :maxdepth: 1
   	
   installation/installation.rst
   installation/distribution.rst
   
.. toctree::
   :caption: USAGE
   :maxdepth: 1

   usage/usage.rst
   usage/app_options.rst		

.. toctree::
   :caption: EXAMPLES
   :maxdepth: 2

   tutorials/tutorials_index.rst

.. toctree::
   :caption: API REFERENCE
   :maxdepth: 2

   api/image_index.rst
   api/imageio_index.rst
   api/imagefilt_index.rst
   api/imageproc_index.rst
   api/source_index.rst
   api/sourceio_index.rst
   api/config_index.rst
   api/logging_index.rst
   api/utils_index.rst
		

.. toctree::
   :caption: PROJECT INFO
   :maxdepth: 2

   info/citing.rst
   info/faq.rst
   info/license.rst

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`



