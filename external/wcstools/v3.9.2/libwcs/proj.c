...

{
   ...
   if (prj->r0 == 0.0) {
      prj->r0 = R2D;
      //prj->w[0] = 360.0/PI;
			prj->w[0] = 360.0/WCS_PI;
      
      //prj->w[1] = PI/360.0;
			prj->w[1] = WCS_PI/360.0;
   } else {
      prj->w[0] = 2.0*prj->r0;
      prj->w[1] = 1.0/prj->w[0];
   }
...

{
   ...
   if (fabs(z) > 1.0) {
      ...
      //z = copysgn (1.0,z) + y0*s/PI;
		  z = copysgn (1.0,z) + y0*s/WCS_PI;
   } else {
      //z = asin(z)*prj->w[4] + y0*s/PI;
      z = asin(z)*prj->w[4] + y0*s/WCS_PI;
   }
