...
find_program (RSCRIPT_EXECUTABLE
               ...
               DOC "Path to the R command script"
             )
...
set (R_PACKAGES)
if (R_EXECUTABLE)
	message (STATUS "Looking for R packages ...")
  foreach (_component ${R_FIND_COMPONENTS})
		message (STATUS "Looking for R package ${_component}")
...
#       endif ()
		...
		if(R_${_component}_FOUND)
			if (${_component} STREQUAL "RInside")
					message (STATUS "RInside package detected!")
					execute_process(COMMAND ${RSCRIPT_EXECUTABLE} -e "RInside:::CxxFlags()" OUTPUT_VARIABLE RINSIDE_CPPFLAGS)
					execute_process(COMMAND ${RSCRIPT_EXECUTABLE} -e "RInside:::LdFlags()" OUTPUT_VARIABLE RINSIDE_LDFLAGS)
					STRING(REGEX REPLACE "-I" "" RINSIDE_CPPFLAGS ${RINSIDE_CPPFLAGS})
					list(APPEND R_INCLUDE_DIR ${RINSIDE_CPPFLAGS})
					list(APPEND R_LIBRARIES ${RINSIDE_LDFLAGS})
					
					message (STATUS "RINSIDE CPPFLAGS: ${RINSIDE_CPPFLAGS}, LDFLAGS: ${RINSIDE_LDFLAGS}")	
			elseif (${_component} STREQUAL "Rcpp")
					message (STATUS "RCPP package detected!")
					execute_process(COMMAND ${RSCRIPT_EXECUTABLE} -e "Rcpp:::CxxFlags()" OUTPUT_VARIABLE RCPP_CPPFLAGS)
					execute_process(COMMAND ${RSCRIPT_EXECUTABLE} -e "Rcpp:::LdFlags()" OUTPUT_VARIABLE RCPP_LDFLAGS)
					STRING(REGEX REPLACE "-I" "" RCPP_CPPFLAGS ${RCPP_CPPFLAGS})
					list(APPEND R_INCLUDE_DIR ${RCPP_CPPFLAGS})
					list(APPEND R_LIBRARIES ${RCPP_LDFLAGS})
					
					message (STATUS "RCPP CPPFLAGS: ${RCPP_CPPFLAGS}, LDFLAGS: ${RCPP_LDFLAGS}")	
				endif()
		endif()
