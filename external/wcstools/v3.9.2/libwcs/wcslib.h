...
typedef struct poly
  ...
  double        *basis;//[ncoeff]         /* Current values of the basis functions */
  double        *coeff;//[ncoeff]         /* Polynom coefficients */ 
  int           ndim;           /* dimensionality of the polynom */
	int           *group;//[ndim]         /* Groups */
  int           ngroup;         /* Number of different groups */
	int           *degree;//[ngroup]        /* Degree in each group */
  }     polystruct;
...
struct linprm {
   ...
   /* Intermediates. //[naxes * naxes * sizeof(double)] */
	 int naxis_sqr;
   double *piximg;//[naxis_sqr]
   double *imgpix;//[naxis_sqr]
};
...
#define WCS_PI	3.141592653589793238462643
#define D2R	WCS_PI/180.0
#define R2D	180.0/WCS_PI
