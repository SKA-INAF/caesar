...
#include "rtnorm.h"
#include "rtnorm_tables.h"

namespace RtNorm_ns {

int RtNorm::N = 4001;   // Index of the right tail
    ...
    const double sigma)
{
  ...
  int xsize=sizeof(RtNorm_ns::x)/sizeof(double);   // Length of table x
  int stop = false;
  ...
  if(a>=b)
  {
    std::cerr<<"RtNorm::rtnorm(): ERROR: B must be greater than A !"<<std::endl;
    return -1;
  }

  // Check if |a| < |b|
  else if(fabs(a)>fabs(b))
    -get_random(r,gen,-b,-a);
  ...
  else
  {
    ...
    while(!stop)
    {
      ...
      else if ((k <= ka+1) || (k>=kb-1 && b<xmax))
      {
        ...
        sim = RtNorm_ns::x[k] + (RtNorm_ns::x[k+1]-RtNorm_ns::x[k]) * gsl_rng_uniform(gen);

        if ((sim >= a) && (sim <= b))
        {
          // Accept this proposition, otherwise reject
          simy = RtNorm_ns::yu[k]*gsl_rng_uniform(gen);
      ...
      {    
        ...
        simy = RtNorm_ns::yu[k] * u;
        d = RtNorm_ns::x[k+1] - RtNorm_ns::x[k];
        ylk = yl(k);
        if(simy < ylk)  // That's what happens most of the time 
        {  
          r = RtNorm_ns::x[k] + u*d*RtNorm_ns::yu[k]/ylk;  
          stop = true;
        }
        else
        {
          sim = RtNorm_ns::x[k] + d * gsl_rng_uniform(gen);
...
double RtNorm::rtexp(gsl_rng *gen, double a, double b)
{
  int stop = false;
