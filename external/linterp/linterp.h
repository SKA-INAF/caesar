...
public:
  ...
  void set_f_array(IterT f_begin, IterT f_end, bool bCopy) {
		//std::cout<<"set_f_array(): Started"<<std::endl;
		
    unsigned int nGridPoints = 1;
		array<int,N> the_sizes;
		//std::cout<<"set_f_array(): Start loop m_grid_list size="<<m_grid_list.size()<<std::endl;
		
		for (unsigned int i=0; i<m_grid_list.size(); i++) {
	  	the_sizes[i] = m_grid_list[i].size();
			nGridPoints *= the_sizes[i];
			//std::cout<<"set_f_array(): Start loop sizes["<<i<<"]="<<the_sizes[i]<<std::endl;
		}
		//std::cout<<"set_f_array(): nGridPoints="<<nGridPoints<<std::endl;
		...
		//std::cout<<"set_f_array(): Start loop m_grid_list size="<<m_grid_list.size()<<" m_bContinuous="<<m_bContinuous<<std::endl;
		...
		m_F_copy.clear();
    if (bCopy == false) {
			//std::cout<<"set_f_array(): bCopy=false, Resetting array..."<<std::endl;
	  	m_pF.reset(new array_type(f_begin, the_sizes));
		} 	
		else {
			//std::cout<<"set_f_array(): bCopy=true, Resetting array of length="<<f_len<<"..."<<std::endl;
	 		m_F_copy = vector<T>(f_begin, f_end);	
			//std::cout<<"set_f_array(): bCopy=true, pto 1: "<<std::endl;
	  	m_pF.reset(new array_type(&m_F_copy[0], the_sizes));
			//std::cout<<"set_f_array(): bCopy=true, pto 2"<<std::endl;
		}
		//std::cout<<"set_f_array(): End"<<std::endl;
